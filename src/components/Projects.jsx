...
const Projects = () => {
  const [activeCompany, setActiveCompany] = useState('NTS Japan');
  ...
  const currentProjects = groupedProjects[activeCompany] || [];
      ...
      <div className="container">
        ...
        <div className="company-nav">
          {Object.keys(groupedProjects).map((company) => (
            <button
              key={company}
              className={`button ${activeCompany === company ? 'button-primary' : 'button-outline'}`}
              onClick={() => {
        ...
        <div className="project-detail">
          <h3>{currentProjects[activeProject]?.title}</h3>
          <p className="project-period">{currentProjects[activeProject]?.period}</p>
          <p className="project-description">{currentProjects[activeProject]?.description}</p>
          <h4>担当業務:</h4>
          <ul className="project-tasks">
            {currentProjects[activeProject]?.tasks.map((task, idx) => (
              <li key={idx}>{task}</li>
            ))}
          </ul>
          <div className="project-meta">
            <p><strong>環境:</strong> {currentProjects[activeProject]?.environment}</p>
            <p><strong>役割:</strong> {currentProjects[activeProject]?.role}</p>
          </div>
        </div>
      </div>
    </section>
